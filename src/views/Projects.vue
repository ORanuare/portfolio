...
<template>
    <div class="flex justify-end flex-wrap overflow-hidden">
        <!-- Projects cards -->
        <div v-for="(project, index) in projects" :key="index" class="w-full sm:w-[50%] md:w-[85%] xl:w-[50%] bg-bones dark:bg-carbon cursor-pointer group relative" @click="openProject(project.link)">
            <!-- Project cover image -->
            ...
            <div class="top-0 absolute opacity-0 border-2 border-transparent group-hover:border-gray-300/20 dark:group-hover:border-gray-300/5 group-hover:opacity-100 w-full h-full duration-300 bg-bones dark:bg-carbon p-8 flex flex-col justify-between">
                ...
                <p class="text-xs lg:text-base xl:text-xs 2xl:text-lg text-carbon/90 dark:text-bones/90">{{ $t('projects-list.' + project.name + '.description') }}</p>
